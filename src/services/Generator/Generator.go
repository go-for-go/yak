...
func (g *Gen) generateBase(f *jen.File) {
	...
	var imps []*importsStructure
	if g.hasPg {
		name := "pg"
		path := "github.com/go-pg/pg"
		if g.Pg10 {
			name = "pg/v10"
			path = "github.com/go-pg/pg/v10"
		}

		imps = append(imps, &importsStructure{
			Name: name,
			Path: path,
		})
	}
	...
	for _, imp := range imps {
		imports.Add(jen.Id("").Lit(imp.Path), jen.Line())
	}
	...
	if joined == "pg" {
		...
		f.Func().Params(jen.Id("s").Id(structPg)).Id(pgFunc).Params().Op("*").Id("pg.Options").BlockFunc(func(g *jen.Group) {
			g.Return(jen.Op("&").Id("pg.Options").Values(jen.Dict{
				jen.Id("User"):     jen.Id("s").Dot("User").Call(),
				jen.Id("Database"): jen.Id("s").Dot("Database").Call(),
				jen.Id("Password"): jen.Id("s").Dot("Pass").Call(),
				jen.Id("Addr"):     jen.Id("s").Dot("Host").Call(),
			}))
		}).Line()
	}
	return
